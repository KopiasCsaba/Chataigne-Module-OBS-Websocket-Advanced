...
function wsMessageReceived(message) {
    ...
    if (d.eventType !== undefined) {
        apiSection = "events";
        name = d.eventType;
    } else if (d.requestType !== undefined) {
        apiSection = "requests";
        name = d.requestType;
    }

    if (apiSection != null) {
        if (!handleCall(apiSection, name, d)) {
            script.log("Message wasn't handled: " + message);
        }
    }
 ...
 * @returns {boolean}
 */
function handleCall(apiSection, name, d) {
    var isRequest = apiSection === "requests";
    var nameKey = isRequest ? "requestType" : "eventType";
    var fieldKey = isRequest ? "responseFields" : "dataFields";
    var valueKey = isRequest ? "responseData" : "eventData";
    var mergeableKey = isRequest ? 0 : 1;
    var containerTitle = isRequest ? "Responses":"Events";

    // Check for custom handlers
    var customHandler = isRequest ? customResponseHandlers[name] : customEventHandlers[name];
    if (customHandler) {
        return customHandler(d);
    }

    script.log("Handling: ", apiSection, name, JSON.stringify(d,null," "));

    // Find the event definition in the API.
    var def = null;
    for (var i = 0; i < API[apiSection].length; i++) {
        if (API[apiSection][i][nameKey] == name) {
            def = API[apiSection][i];
            break;
        }
    }

    if (def == null) {
        script.log("Unknown call arrived that we can't handle: " + apiSection + ":" + name);
        return false;
    }
    ...
    var eventContainer = getContainer(containerTitle + "#" + name);
    eventContainer.clear();

    var fields = def[fieldKey];

    // Add parameters
    for (var i = 0; i < fields.length; i++) {
        var fieldDef = fields[i];
        var fieldValue = d[valueKey][fieldDef.valueName];

        if (fieldDef.valueType == "Array<Object>") {
            renderArray("events." + name + "." + fieldDef.valueName, fieldValue, eventContainer);
        } else {
            addParameter(fieldDef.valueName, fieldValue, fieldDef.valueType, eventContainer, fieldDef.valueDescription);
        }

        // Look for merged fields and do the same...
        for (var m = 0; m < mergeableRequestsAndEvents.length; m++) {
            if (mergeableRequestsAndEvents[m][mergeableKey] == name + "." + fieldDef.valueName) {
                ...
                if (fieldDef.valueType == "Array<Object>") {
                    renderArray("events." + name + "." + fieldDef.valueName, fieldValue, subContainer);
                } else {
                    addParameter(fieldLabel, fieldValue, fieldDef.valueType, subContainer, fieldDef.valueDescription);
                }
            }
        }
